--[[****************************************************************************
  ...
  * _NPCScanGold.Button.lua - Displays a button to target found NPCs.              *
  ****************************************************************************]]


local _NPCScanGold = select( 2, ... );
local me = CreateFrame( "Button", "_NPCScanGoldButton", UIParent, "SecureActionButtonTemplate,SecureHandlerShowHideTemplate" );
_NPCScanGold.Button = me;
...
function me.PlaySound ( AlertSound )
	...
	if ( _NPCScanGold.Options.AlertSoundUnmute ) then
...
function me:SetNPC ( ID, Name )
	if ( tonumber( ID ) ) then
		ID = tonumber( ID );
		_NPCScanGold.Overlays.Add( ID );
		_NPCScanGold.Overlays.Found( ID );
	end

	self.PlaySound( _NPCScanGold.Options.AlertSound );
	...
	if ( InCombatLockdown() ) then
		if ( type( self.PendingID ) == "number" ) then -- Remove old pending NPC
			_NPCScanGold.Overlays.Remove( self.PendingID );
		end
...
function me:Update ( ID, Name )
	if ( type( self.ID ) == "number" ) then -- Remove last overlay
		_NPCScanGold.Overlays.Remove( self.ID );
	end
	self.ID = ID;
...
function me:OnShow ()
	...
	self:EnableDrag( IsModifiedClick( "_NPCSCANGOLD_BUTTONDRAG" ) );
end
...
SubTitle:SetText( _NPCScanGold.L.BUTTON_FOUND );
...
me:SetScript( "OnEvent", _NPCScanGold.Frame.OnEvent );
