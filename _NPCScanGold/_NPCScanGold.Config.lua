--[[****************************************************************************
  ...
  * _NPCScanGold.Config.lua - Adds an options pane to the Interface Options menu.  *
  ****************************************************************************]]
...
me.AlertSoundUnmute = CreateFrame( "CheckButton", "_NPCScanGoldConfigUnmuteCheckbox", AlertOptions, "InterfaceOptionsCheckButtonTemplate" );
me.AlertSound = CreateFrame( "Frame", "_NPCScanGoldConfigSoundDropdown", AlertOptions, "UIDropDownMenuTemplate" );
...
function me.Test:OnClick ()
	...
	_NPCScanGold.Print( L.FOUND_FORMAT:format( Name ), GREEN_FONT_COLOR );
	_NPCScanGold.Print( L.CONFIG_TEST_HELP_FORMAT:format( GetModifiedClick( "_NPCSCANGOLD_BUTTONDRAG" ) ) );

	_NPCScanGold.Button:SetNPC( "player", Name );
end
--- Sets the AlertSoundUnmute option when its checkbox is clicked.
function me.AlertSoundUnmute.setFunc ( Enable )
	_NPCScanGold.SetAlertSoundUnmute( Enable == "1" );
end
--- Sets an alert sound chosen from the LibSharedMedia dropdown.
function me.AlertSound:OnSelect ( NewValue )
	_NPCScanGold.Button.PlaySound( NewValue ); -- Play sample
	_NPCScanGold.SetAlertSound( NewValue );
end
--- Builds a dropdown menu for alert sounds with LibSharedMedia options.
function me.AlertSound:initialize ()
	local Value = _NPCScanGold.Options.AlertSound;
...
function me:default ()
	_NPCScanGold.Synchronize(); -- Resets all
end
