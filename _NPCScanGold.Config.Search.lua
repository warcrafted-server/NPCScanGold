--[[****************************************************************************
  ...
  * _NPCScanGold.Config.Search.lua - Adds a configuration pane to add/remove NPCs  *
  *   and achievements to search for.                                          *
  ****************************************************************************]]


local _NPCScanGold = select( 2, ... );
local L = _NPCScanGold.L;
local me = CreateFrame( "Frame" );
_NPCScanGold.Config.Search = me;

me.AddFoundCheckbox = CreateFrame( "CheckButton", "_NPCScanGoldSearchAchievementAddFoundCheckbox", me, "InterfaceOptionsCheckButtonTemplate" );

me.TableContainer = CreateFrame( "Frame", nil, me );

me.NPCControls = CreateFrame( "Frame", nil, me.TableContainer );
me.NPCName = CreateFrame( "EditBox", "_NPCScanGoldSearchNpcName", me.NPCControls, "InputBoxTemplate" );
me.NPCNpcID = CreateFrame( "EditBox", "_NPCScanGoldSearchNpcID", me.NPCControls, "InputBoxTemplate" );
me.NPCWorld = CreateFrame( "EditBox", "_NPCScanGoldSearchNpcWorld", me.NPCControls, "InputBoxTemplate" );
me.NPCWorldButton = CreateFrame( "Button", nil, me.NPCWorld );
me.NPCWorldButton.Dropdown = CreateFrame( "Frame", "_NPCScanGoldSearchNPCWorldDropdown", me.NPCWorldButton );
...
function me.AddFoundCheckbox.setFunc ( Enable )
	if ( _NPCScanGold.SetAchievementsAddFound( Enable == "1" ) ) then
		_NPCScanGold.CacheListPrint( true );
	end
end
...
local function GetWorldID ( World )
	if ( World ~= "" ) then
		return _NPCScanGold.ContinentIDs[ World ] or World;
	end
end
...
function me:TabOnEnter ()
	...
	if ( self.AchievementID ) then
		...
		local WorldID = _NPCScanGold.Achievements[ self.AchievementID ].WorldID;
		...
		if ( not _NPCScanGold.OptionsCharacter.Achievements[ self.AchievementID ] ) then
...
function me:TabCheckOnClick ()
	...
	if ( not Enable ) then
		_NPCScanGold.AchievementRemove( AchievementID );
	elseif ( _NPCScanGold.AchievementAdd( AchievementID ) ) then -- Cache might have changed
		_NPCScanGold.CacheListPrint( true );
	end
end
...
function me.NPCValidate ()
	...
	local OldName = _NPCScanGold.OptionsCharacter.NPCs[ NpcID ];
	local OldWorldID = _NPCScanGold.OptionsCharacter.NPCWorldIDs[ NpcID ];
	local CanAdd = NpcID and Name ~= ""
		and NpcID >= 1 and NpcID <= _NPCScanGold.NpcIDMax
...
function me.NPCAdd:OnClick ()
	local NpcID, Name, WorldID = me.NPCNpcID:GetNumber(), me.NPCName:GetText(), GetWorldID( me.NPCWorld:GetText() );
	if ( _NPCScanGold.TamableIDs[ NpcID ] ) then
		_NPCScanGold.Print( L.SEARCH_ADD_TAMABLE_FORMAT:format( Name ) );
	end
	_NPCScanGold.NPCRemove( NpcID );
	if ( _NPCScanGold.NPCAdd( NpcID, Name, WorldID ) ) then
		_NPCScanGold.CacheListPrint( true );
	end
	me.NPCClear();
end
...
function me.NPCRemove:OnClick ()
	_NPCScanGold.NPCRemove( me.NPCNpcID:GetNumber() );
	me.NPCClear();
end
...
function me:NPCOnSelect ( NpcID )
	if ( NpcID ~= nil ) then
		...
		me.NPCName:SetText( _NPCScanGold.OptionsCharacter.NPCs[ NpcID ] );
		me.NPCWorld:SetText( GetWorldIDName( _NPCScanGold.OptionsCharacter.NPCWorldIDs[ NpcID ] ) or "" );
	end
end
...
function me.NPCWorldButton.Dropdown:initialize ()
	...
	if ( not _NPCScanGold.ContinentIDs[ CurrentWorld ] ) then -- Add current instance name
		-- Spacer
...
function me:NPCUpdate ()
	me.NPCValidate();
	local WorldIDs = _NPCScanGold.OptionsCharacter.NPCWorldIDs;
	for NpcID, Name in pairs( _NPCScanGold.OptionsCharacter.NPCs ) do
		local Row = me.Table:AddRow( NpcID,
			_NPCScanGold.TestID( NpcID ) and [[|TInterface\RaidFrame\ReadyCheck-NotReady:0|t]] or nil,
			Name, NpcID, GetWorldIDName( WorldIDs[ NpcID ] ) );

		if ( not _NPCScanGold.NPCIsActive( NpcID ) ) then
...
function me:AchievementUpdate ()
	local Achievement = _NPCScanGold.Achievements[ self.AchievementID ];
	for CriteriaID, NpcID in pairs( Achievement.Criteria ) do
		...
		if ( not _NPCScanGold.AchievementNPCIsActive( Achievement, NpcID ) ) then
			Row:SetAlpha( me.InactiveAlpha );
		end
	end
end
...
function me:AchievementActivate ()
	...
	me.Table.Header:SetAlpha( _NPCScanGold.OptionsCharacter.Achievements[ self.AchievementID ] and 1.0 or me.InactiveAlpha );
end
...
do
	...
	function me:TableCreate ()
		-- Note: Keep late bound so _NPCScanGold.Overlay can hook into the table as it's created
...
function me:default ()
	_NPCScanGold.Synchronize( _NPCScanGold.Options ); -- Resets only character settings
end
...
me.NPCAdd:SetScript( "OnEnter", _NPCScanGold.Config.ControlOnEnter );
...
local function EditBoxSetup ( self )
	...
	self:SetScript( "OnEnter", _NPCScanGold.Config.ControlOnEnter );
...
NpcID:SetMaxLetters( floor( log10( _NPCScanGold.NpcIDMax ) ) + 1 );
...
local function AddTab ( ID, Update, Activate, Deactivate )
	TabCount = TabCount + 1;
	local Tab = CreateFrame( "Button", "_NPCScanGoldSearchTab"..TabCount, me.TableContainer, "TabButtonTemplate" );
	Tabs[ ID ] = Tab;
...
for AchievementID in pairs( _NPCScanGold.Achievements ) do
